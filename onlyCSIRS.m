...
usersInfo.num=16;
usersInfo.eachofPorts=2;
usersInfo.nRxAntennas=usersInfo.num*usersInfo.eachofPorts;
usersInfo.numBw=5;
...
trdataAll=zeros(22152,usersInfo.nRxAntennas,usersInfo.numBw);%普通cp
usersInfo.num=16;
...
for snrIdx = 1:numel(SNRIn)
    ...
    DMRSIndices = [];
    PDSCHIndices = [];
    Peak = zeros(usersInfo.nRxAntennas,1);
    RMS = zeros(usersInfo.nRxAntennas,1);
    pdschSymbolsNum = gnb.NDLRB*12*12-gnb.NDLRB*12*(4+2);
    pdschSymbols = zeros(pdschSymbolsNum,usersInfo.nRxAntennas,usersInfo.numBw);
    SNRdB = SNRIn(snrIdx);
    %产生八层PDSCH数据
    for layer = 1:usersInfo.nRxAntennas
        for BW = 1:usersInfo.numBw
...
%     CSIRS = CSIRSgenerator(gnb, 1);
    CSIRS = CSIRSgenerator_old(gnb, 1);
    %generate DMRS on 12 antenna
    DMRS = DMRSgenerator(gnb, 1);
    
    %%映射参考信号
    for port = 1:usersInfo.nRxAntennas
        temp = [];
...
%             [pdschGrid,csiisindices] = CSIRSmapping_new_15812(pdschGrid,CSIRS(ind1,:),gnb,port);
            [pdschGrid,csiisindices] = CSIRSmapping_new_891011(pdschGrid,CSIRS(ind1,:),gnb,port);
             %DMRS mapping
            if port>=1 && port<=12              
...
%             pdschGrid = CSIRSmapping_new_15812(pdschGrid,CSIRS(ind1,:),gnb,port);
            [pdschGrid,~] = CSIRSmapping_new_891011(pdschGrid,CSIRS(ind1,:),gnb,port);
...
%             hD = ChannelEstimate_new_15812(grid,txgrid,CSIRSIndices);
          ...
          if (nn==1)
            % 信道估计
%             hD = ChannelEstimate_new_15812(grid,txgrid,CSIRSIndices);
%             hD = ChannelEstimate_new_15812_alleles(grid,txgrid,CSIRSIndices,DMRSIndices);
            hD = ChannelEstimate_new_891011(grid,txgrid,CSIRSIndices);%1320x12x32x32
...
%             DL_precoding = zeros(32, 2, usersInfo.num);%32x2x16
...
%                         DL_precoding_all_temp(i,t,:,(k-1)*2 + (1:2),BW)= W_DL_k./sqrt(2);  %把所有用户预编码拼成2行 , 1320x12x2x32
                        DL_precoding(i,t,:,(k-1)*2 + (1:2))= W_DL_k./sqrt(2);
                    end
                end  %end for k=1:uenum
            end % end for 1:12:num_n
            for ii=1:110  
                DL_precoding_all_temp( (ii-1)*12+(1:12),:,:,:,BW)=repmat(DL_precoding((ii-1)*12+1,:,:,:),12,1,1,1);
            end
            fprintf('预编码的计算已完成 .\n');
%}  
%
...
%             WVtmp=permute(DL_precoding_all_temp,[3 4 1 2 5]);%32x32x1320x14
            WVtmp=DL_precoding_all_temp(:,:,:,:,BW);% 1320x12x32x32x5
            WV=permute(WVtmp,[3 4 1 2]);%32x32x1320x12
...
%             pdschTx = ExtractResources(PDSCHIndices,txgrid);
            if nn==1
%             chEst = ExtChResponse(hD, PDSCHIndices)/9; %ori
            chEst = ExtChResponse(hD, PDSCHIndices); %new
...
%             pdschRxEq_all(:,:,BW)=pdschRxEq;%16720x32
              for port=1:32
                  [rxCws,~] = ltePDSCHDecode_modify(gnb, pdsch, pdschRxEq(:,port));%8360x1
