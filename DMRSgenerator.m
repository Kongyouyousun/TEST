function y = DMRSgenerator(gnb, numTx)
...
% Assumed parameters
nS = gnb.NSubframe;
NSCID = randi([0 65535],1);        % One of possible 65536 values
SCID = randi([0 1],1);
NmaxDL_RB = gnb.NDLRB;    % largest downlink bandwidth configuration, in resource blocks
y = complex(zeros(4*NmaxDL_RB*5, 1,numTx));
l = [4 5];     % OFDM symbol idx in a slot for common first antenna port
...
for port = 1: numTx
    for i = 1 % slot wise
        for lIdx = 1:1 % symbol wise
            c_init = mod((2^17)*(14*nS/2+l(lIdx)+1)*(2*NSCID+1) + 2*NSCID+SCID,2^31);
