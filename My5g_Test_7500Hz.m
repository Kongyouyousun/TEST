...
for snrIdx = 1:numel(SNRIn)
    ...
    Peak = zeros(32,1);
    RMS = zeros(32,1);
    pdschSymbolsNum = gnb.NDLRB*12*14-gnb.NDLRB*72;
    pdschSymbols = zeros(pdschSymbolsNum,32,10);
    %产生八层PDSCH数据
    for layer = 1:32
        for BW = 1:10
...
%             reset(channel);
    ...
    for port = 1:32
        temp = [];
        pdschIndices = 1:14*gnb.NDLRB*12;   
        for BW = 1:10           
            % PDSCH mapping in grid associated with PDSCH transmission period
            pdschGrid = lteDLResourceGrid(gnb);

            if BW == 1
                ind1 = 1:length(CSIRS)/10;
                ind2 = 1:length(DMRS)/10;
            else
                ind1 = ind1 + length(CSIRS)/10;
                ind2 = ind2 + length(DMRS)/10;
            end
            %CSI-RS mapping
...
%         pdschIndices = ExpungeFrom(pdschIndices,csiisindices);
    ...
    %映射PDSCH数据
    txGrid = zeros(gnb.NDLRB*12*10,size(pdschGrid,2),32);
     for port = 1:32
        for BW = 1:10            
            % PDSCH mapping in grid associated with PDSCH transmission period
            pdschGrid = lteDLResourceGrid(gnb);
    ...
    step(hSpec1,txWave(:,1));    
        ...
        %       COST 207
        channeltype = 2;
        Cost=cost_207(channeltype,1/122.88e6,3);
%         for antNo = 1:32
...
%     foffset = 200.0;         % Frequency offset in Hertz
%     fprintf('add frequency error: %0.1fHz\n',foffset);
%     t = (0:length(rxWaveform)-1).'/122.88e6;
%     rxWaveform = rxWaveform.* repmat(exp(1i*2*pi*foffset*t),1,32);
%     %detect frequency error
%     foffset_est = mylteFrequencyOffset_7500Hz(rxWaveform);
...
%      fprintf('Frequency offset error after FrequencyCorrect : %0.2fHz\n',foffset_est);
                ...
                % resource grid
                rxGrid = OFDMDemodulate_7500Hz(gnb, rxWaveform);  
%                 sigma2=1/(10^(SNRdB/10));
%             for i=1:12
%             noise_srs=sqrt(sigma2/2)* (randn(size(rxGrid(:,i,:)))+1i*randn(size(rxGrid(:,i,:)))); %1320x32
%             rxGrid(:,i,:)=rxGrid(:,i,:)+noise_srs;
%             end

        lenGrid = size(rxGrid,1)/10;
        pdschRx = [];
        pdschTx = [];
        pdschRxEq = [];
         for CC=1:32
             evm(CC) = struct('Peak',0 ,'RMS',0);
        end
        for BW = 1:10
...
%             hD = ChEst_freq_mmseforCSIrs(EsNO, rmcInfo.NDLRB, rmcInfo.Nfft,chinfo.PathSampleDelays,grid,txgrid,CSIRSIndices);
            chEst = ExtChResponse(hD, PDSCHIndices)/9; 
